...
public class ShippingPage extends JFrame {
    ...
    private JSplitPane createSummarySplitPane() {
        ...
        try {
            // 1. 이미지 경로 설정
            ImageIcon mapIcon = new ImageIcon(getClass().getResource("/guitool/image/korea.png"));

            if (mapIcon.getImage() == null) {
                throw new Exception("이미지 리소스를 찾을 수 없습니다.");
            }

            // ★★★ [핵심 수정] 높이(500)를 기준으로 크기 조절 (너비는 -1로 자동) ★★★
            // 창 높이가 600이므로 500 정도로 맞추면 잘리지 않습니다.
        ...
        // 높이 기준 리사이징을 했으므로 분할 위치를 적절히 조정 (약 400~450 정도)
